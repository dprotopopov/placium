...
namespace Route.Algorithms.Collections
{
    ...
    public class HugeDictionary<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        /// </summary>
        public int CountDictionaries => _dictionary.Count;
        ...
        public void Add(TKey key, TValue value)
        {
            ...
            var added = false;
            for (var idx = 0; idx < _dictionary.Count; idx++)
            {
        ...
        public ICollection<TKey> Keys
        {
            get
            {
                ...
                for (var i = 0; i < enumerables.Length; i++) enumerables[i] = _dictionary[i].Keys;
        ...
        public bool Remove(TKey key)
        {
            for (var idx = 0; idx < _dictionary.Count; idx++)
                ...
                {
                    if (_dictionary[idx].Count == 0 && _dictionary.Count > 1) _dictionary.RemoveAt(idx);
                    return true;
                }
        ...
        public ICollection<TValue> Values
        {
            get
            {
                ...
                for (var i = 0; i < enumerables.Length; i++) enumerables[i] = _dictionary[i].Values;
        ...
        public int Count
        {
            ...
            {
                var total = 0;
                for (var idx = 0; idx < _dictionary.Count; idx++) total = total + _dictionary[idx].Count;
                return total;
            }
        }
        ...
        {
            if (_dictionary.Count == 0) return Enumerable.Empty<KeyValuePair<TKey, TValue>>().GetEnumerator();
            IEnumerable<KeyValuePair<TKey, TValue>> enumerable = _dictionary[0];
            for (var i = 1; i < _dictionary.Count; i++) enumerable = enumerable.Concat(_dictionary[i]);
        ...
        private class ReadonlyEnumerableCollection<T> : ICollection<T>
        {
            ...
            public ReadonlyEnumerableCollection(params IEnumerable<T>[] enumerables)
            {
                ...
                for (var i = 1; i < enumerables.Length; i++) _enumerable = _enumerable.Concat(enumerables[i]);
            }

            int ICollection<T>.Count => _enumerable.Count();
