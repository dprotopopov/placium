    ...
    public class UnixSocketAuthenticationHandler : AuthenticationHandler<UnixSocketAuthenticationOptions>
    {
        ...
        protected virtual Task<AuthenticateResult> HandleAuthenticateAsync(IUnixSocketCredentialFeature feature)
        {
            ...
            try
            {
                ...
                identity.AddClaim(new Claim(ClaimTypes.NameIdentifier, feature.UserId.ToString(CultureInfo.InvariantCulture)));
                identity.AddClaim(new Claim(nameof(feature.ProcessId), feature.ProcessId.ToString(CultureInfo.InvariantCulture)));
                identity.AddClaim(new Claim(nameof(feature.GroupId), feature.GroupId.ToString(CultureInfo.InvariantCulture)));
            ...
            {
                _logger.LogError(ex, "In Task<AuthenticateResult> HandleAuthenticateAsync(IUnixSocketCredentialFeature feature)");
                throw ex;
