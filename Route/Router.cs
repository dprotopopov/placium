...
using Route.Algorithms;
using Route.Algorithms.Contracted.Dual;
using Route.Algorithms.Contracted.Dual.ManyToMany;
using Route.Algorithms.Contracted.EdgeBased;
using Route.Algorithms.Default;
using Route.Algorithms.Routes;
using Route.Algorithms.Search;
using Route.Algorithms.Search.Cache;
            ...
            try
            {
                ...
                { // use the contracted graph.
                    ...
                    { // use vertex-based graph for edge-based routing.
                        ...
                        var bidirectionalSearch = new Algorithms.Contracted.BidirectionalDykstra<T>(contracted.NodeBasedGraph, null, weightHandler,
                    ...
                    { // use node-based routing.
                        var bidirectionalSearch = new Algorithms.Contracted.BidirectionalDykstra<T>(contracted.NodeBasedGraph, _db.GetRestrictions(profileInstance.Profile), weightHandler,
                ...
                { // use the regular graph.
                    ...
                    {
                        var sourceSearch = new Algorithms.Default.Dykstra<T>(_db.Network.GeometricGraph.Graph, _db.GetGetSimpleRestrictions(profileInstance.Profile), weightHandler,
                            source.ToEdgePaths(_db, weightHandler, true), maxSearch, false);
                        var targetSearch = new Algorithms.Default.Dykstra<T>(_db.Network.GeometricGraph.Graph, _db.GetGetSimpleRestrictions(profileInstance.Profile), weightHandler,
                            target.ToEdgePaths(_db, weightHandler, false), maxSearch, true);

                        var bidirectionalSearch = new Algorithms.Default.BidirectionalDykstra<T>(sourceSearch, targetSearch, weightHandler);
                        bidirectionalSearch.Run(cancellationToken);
                        if (!bidirectionalSearch.HasSucceeded)
                        {
                            if (path == null)
            ...
            RoutingSettings<T> settings, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (useContracted)
                {
                    if (contracted.HasEdgeBasedGraph)
                    { // use edge-based routing.
                        var algorithm = new ManyToManyBidirectionalDykstra<T>(_db, profileInstance.Profile, weightHandler,
                        ...
                        contracted.NodeBasedIsEdgedBased)
                    { // use vertex-based graph for edge-based routing.
                        var algorithm = new VertexToVertexAlgorithm<T>(contracted.NodeBasedGraph, weightHandler,
                            Algorithms.Contracted.Dual.RouterPointExtensions.ToDualDykstraSources(sources, _db, weightHandler, true),
                            Algorithms.Contracted.Dual.RouterPointExtensions.ToDualDykstraSources(targets, _db, weightHandler, false), maxSearch);
                        algorithm.Run(cancellationToken);
                    ...
                    { // use node-based routing.
                        var algorithm = new Algorithms.Contracted.ManyToManyBidirectionalDykstra<T>(_db, profileInstance.Profile, weightHandler,
                            sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                ...
                if (paths == null)
                {
                    ...
                    var algorithm = new ManyToMany<T>(_db, weightHandler, sources, targets, maxSearch);
                    algorithm.Run(cancellationToken);
                    if (!algorithm.HasSucceeded)
                    {
                        return new Result<EdgePath<T>[][]>(algorithm.ErrorMessage, (message) =>
            ...
            ISet<int> invalidSources, ISet<int> invalidTargets, RoutingSettings<T> settings, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (useContracted)
                {
                    if (contracted.HasEdgeBasedGraph)
                    { // use edge-based routing.
                        var algorithm = new ManyToManyWeightsBidirectionalDykstra<T>(_db, profileInstance.Profile, weightHandler,
                            sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                        ...
                        contracted.NodeBasedIsEdgedBased)
                    { // use vertex-based graph for edge-based routing.
                        weights = RouterExtensions.CalculateManyToMany(contracted, _db, profileInstance.Profile,
                            weightHandler, sources, targets, maxSearch, cancellationToken).Value;
                    }
                    else
                    { // use node-based routing.
                        var algorithm = new Algorithms.Contracted.ManyToManyWeightsBidirectionalDykstra<T>(_db, profileInstance.Profile, weightHandler,
                            sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                ...
                { // use regular graph.
                    if (_db.HasComplexRestrictions(profileInstance.Profile))
                    {
                        var algorithm = new Algorithms.Default.EdgeBased.ManyToMany<T>(this, weightHandler, _db.GetGetRestrictions(profileInstance.Profile, true), sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                            return new Result<T[][]>(algorithm.ErrorMessage, (message) =>
            ...
            ISet<int> invalidSources, ISet<int> invalidTargets, RoutingSettings<float> settings, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (useContracted)
                {
                    if (contracted.HasEdgeBasedGraph)
                    { // use edge-based routing.
                        var algorithm = new ManyToManyWeightsBidirectionalDykstra<float>(_db, profileInstance.Profile, weightHandler,
                            sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                        ...
                        contracted.NodeBasedIsEdgedBased)
                    { // use vertex-based graph for edge-based routing.
                        weights = RouterExtensions.CalculateManyToMany(contracted,
                            _db, profileInstance.Profile,
                            weightHandler, sources, targets, maxSearch, cancellationToken).Value;
                    }
                    else
                    { // use node-based routing.
                        var algorithm = new Algorithms.Contracted.ManyToManyWeightsBidirectionalDykstra<float>(_db, profileInstance.Profile, weightHandler,
                            sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                ...
                { // use regular graph.
                    if (_db.HasComplexRestrictions(profileInstance.Profile))
                    {
                        var algorithm = new Algorithms.Default.EdgeBased.ManyToMany<float>(this, weightHandler, _db.GetGetRestrictions(profileInstance.Profile, true), sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                            return new Result<float[][]>(algorithm.ErrorMessage, (message) =>
                    ...
                    {
                        var algorithm = new ManyToMany<float>(_db, weightHandler, sources, targets, maxSearch);
                        algorithm.Run(cancellationToken);
                        if (!algorithm.HasSucceeded)
                        {
                            return new Result<float[][]>(algorithm.ErrorMessage, (message) =>
