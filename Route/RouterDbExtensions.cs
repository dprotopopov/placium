...
using Route.Algorithms.Contracted;
using Route.Algorithms.Contracted.Witness;
using Route.Algorithms.Dual;
using Route.Algorithms.Networks.Islands;
...
using Route.Data.Network.Edges;
using Route.Data.Network.Restrictions;
using Route.Graphs;
using Route.Graphs.Directed;
using Route.IO.Json;
using Route.LocalGeo;
using Route.LocalGeo.IO;
using Route.Logging;
using Route.Profiles;
using HierarchyBuilder = Route.Algorithms.Contracted.Dual.HierarchyBuilder;
    ...
    public static class RouterDbExtensions
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (forceEdgeBased)
            {
                ...
                Logger.Log("RouterDb", Logging.TraceEventType.Information,
...
//                hierarchyBuilder.Run(cancellationToken);
                var hierarchyBuilder = 
                    new HierarchyBuilder(contracted, 
...
//                hierarchyBuilder.Run(cancellationToken);
            ...
            where T : struct
        {
            ...
            lock (db) // TODO: reevaluate this lock, not needed around this entire block!
            {
                if (forceEdgeBased)
                { // edge-based is needed when complex restrictions found.
                    Logger.Log("RouterDb", Logging.TraceEventType.Information, "Contracting into an edge-based graph for profile {0}...",
                        profile.FullName);
...
//                    hierarchyBuilder.Run(cancellationToken);
            ...
            bool includeProfileDetails = true)
        {
            ...
            var edgeAttributes = new AttributeCollection(db.EdgeMeta.Get(edgeEnumerator.Data.MetaId));
