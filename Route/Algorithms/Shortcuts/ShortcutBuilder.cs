...
namespace Route.Algorithms.Shortcuts
{
    /// <summary>
    /// A shortcut builder.
    /// </summary>
    public class ShortcutBuilder : AlgorithmBase
    {
        ...
        protected override void DoRun(CancellationToken cancellationToken)
        {
            ...
            var algorithm = new ManyToMany<Weight>(_db, weightHandler, points, points, new Weight()
            {
            ...
            for (var i = 0; i < _locations.Length; i++)
            {
                ...
                for (var j = 0; j < _locations.Length; j++)
                {
                    ...
                    if (algorithm.TryGetPath(i, j, out path))
                    {
                        ...
                        if(pathList.Count < 2)
                        {
                            Logger.Log("ShortcutBuilder", TraceEventType.Warning, "Shortcut consists of only one vertex from {0}@{1} -> {2}@{3}!",
                                i, _locations[i], j, _locations[j]);
                            continue;
                        }
                        if (pathList[0] == pathList[pathList.Count - 1])
                        {
                            Logger.Log("ShortcutBuilder", TraceEventType.Warning, "Shortcut has the same start and end vertex from {0}@{1} -> {2}@{3}!",
                                i, _locations[i], j, _locations[j]);
                            continue;
                        }
                    ...
                    {
                        Logger.Log("ShortcutBuilder", TraceEventType.Warning, "Shortcut not found from {0}@{1} -> {2}@{3}!",
                            i, _locations[i], j, _locations[j]);
                    }
                }
            }
