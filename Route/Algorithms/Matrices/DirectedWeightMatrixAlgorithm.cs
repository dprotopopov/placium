        ...
        where T : struct
    {
        ...
        private void DoEdgeBased(CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < _sourcePaths.Length; i++)
            {
                ...
                if (path != null)
                {
                    var forward = new Contracted.EdgeBased.Dykstra<T>(_graph, _weightHandler, new EdgePath<T>[] { path }, (v) => null, false, _max);
            ...
            for (var i = 0; i < _targetPaths.Length; i++)
            {
                ...
                if (path != null)
                {
                    var backward = new Contracted.EdgeBased.Dykstra<T>(_graph, _weightHandler, new EdgePath<T>[] { path }, (v) => null, true, _max);
        ...
        private void DoDualBased(CancellationToken cancellationToken)
        {
            ...
            var dykstraTargets = DykstraSourceExtensions.ToDykstraSources<T>(sources);
            var algorithm = new VertexToVertexWeightAlgorithm<T>(_dualGraph, _weightHandler,
