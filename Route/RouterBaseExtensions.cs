...
using Route.Algorithms.Default.EdgeBased;
    ...
    public static class RouterBaseExtensions
    {
        ...
        public static Func<ushort, Factor> GetDefaultGetFactor(this RouterBase router, IProfileInstance profileInstance)
        {
            ...
            { // use the regular function, and consult profiles continuously.
                ...
                { // when there is a cache, built it on demand.
                    Logger.Log("RouterBaseExtensions", TraceEventType.Information, "Profile {0} is not cached, building cache.",
                ...
                { // no cache, caching disabled.
                    Logger.Log("RouterBaseExtensions", TraceEventType.Warning, "Profile {0} is not cached, this could slow down routing significantly, consider building a profile cache.",
                        profileInstance.Profile.FullName);
        ...
        public static Func<GeometricEdge, bool> GetIsAcceptable(this RouterBase router, params IProfileInstance[] profiles)
        {
            ...
            { // use the regular function, and consult profiles continuously.
                ...
                { // when there is a cache, built it on demand.
                    ...
                    Logger.Log("RouterBaseExtensions", TraceEventType.Information, "Profile(s) {0} not cached, building cache.",
                ...
                {
                    Logger.Log("RouterBaseExtensions", TraceEventType.Warning,
            ...
            CancellationToken cancellationToken)
        {
            ...
            { // something went wrong.
                ...
                {
                    return new ResolveFailedException(message);
                });
            }
            ...
            where T : struct
        {
            try
            {
                ...
                { // do dual edge-based routing.
                    ...
                    var dykstraSources = DykstraSourceExtensions.ToDykstraSources<T>(sources);
                    var dykstraTargets = DykstraSourceExtensions.ToDykstraSources<T>(targets);
                    var algorithm = new VertexToVertexWeightAlgorithm<T>(graph, weightHandler,
                ...
                { // use uncontracted routing.
                    ...
                    var dykstraSources = DirectedDykstraSourceExtensions.ToDykstraSources<T>(sources);
                    var dykstraTargets = DirectedDykstraSourceExtensions.ToDykstraSources<T>(targets);
                    var algorithm = new DirectedManyToManyWeights<T>(graph, weightHandler, router.Db.GetRestrictions(profileInstance.Profile),
                        dykstraSources, dykstraTargets, maxSearch);
                    algorithm.Run(cancellationToken);
                    if (!algorithm.HasSucceeded)
                    {
            ...
            where T : struct
        {
            ...
            { // make sure angles are normalized.
                sourceDirection = (float)Tools.NormalizeDegrees(sourceDirection.Value);
            }
            if (targetDirection.HasValue)
            { // make sure angles are normalized.
                targetDirection = (float)Tools.NormalizeDegrees(targetDirection.Value);
            }
