...
namespace Placium.Common
{
    public class BaseAppService
    {
        ...
        {
            using (var command = new NpgsqlCommand(
                "INSERT INTO service_history3(service_type3,last_record_number) VALUES (@service_type, @last_record_number) ON CONFLICT (service_type3) DO UPDATE SET last_record_number=EXCLUDED.last_record_number"
                , connection))
            {
                command.Parameters.AddWithValue("service_type", service_type);
                command.Parameters.AddWithValue("last_record_number", last_record_number);
