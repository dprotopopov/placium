...
namespace Route.LocalGeo
{
    ...
    public struct Line
    {
        ...
        public Coordinate Middle =>
            new Coordinate((Coordinate1.Latitude + Coordinate2.Latitude) / 2,
        ...
        public Coordinate? Intersect(Line line)
        {
            ...
            var x = (B * line.C - line.B * C) / det;
            var y = (line.A * C - A * line.C) / det;
            ...
            if (_coordinate1.Elevation.HasValue && _coordinate2.Elevation.HasValue)
            {
                ...
                else if (Math.Abs(A) < E && Math.Abs(B) < E)
                {
                ...
                else
                {
                    ...
                    {
                        var diffLat = Math.Abs(A);
                        var diffLatIntersection = Math.Abs(coordinate.Latitude - _coordinate1.Latitude);

                        coordinate.Elevation = (short)((_coordinate2.Elevation - _coordinate1.Elevation) *
                                                       (diffLatIntersection / diffLat) +
                                                       _coordinate1.Elevation);
                    }
                    else
                    {
                        var diffLon = Math.Abs(B);
                        var diffLonIntersection = Math.Abs(coordinate.Longitude - _coordinate1.Longitude);
        ...
        public Coordinate? ProjectOn(Coordinate coordinate)
        {
            ...
            if (thisLine.Coordinate1.Longitude > thisLine.Coordinate2.Longitude) xLength = -xLength;
            ...
            if (thisLine.Coordinate1.Latitude > thisLine.Coordinate2.Latitude) yLength = -yLength;
            ...
            if (!projected.HasValue) return null;
        ...
        public float? DistanceInMeter(Coordinate coordinate)
        {
            ...
            if (projected.HasValue) return Coordinate.DistanceEstimateInMeter(coordinate, projected.Value);
            return null;
        }
    }
}