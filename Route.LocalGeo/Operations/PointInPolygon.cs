...
namespace Route.LocalGeo.Operations;
...
internal static class PointInPolygon
{
    ...
    internal static bool PointIn(List<Coordinate> ring, Coordinate point)
    {
        ...
        */
        ...
        if (!(bbWest <= longitude && bbNorth >= latitude && bbEast >= longitude && bbSouth <= latitude)) return false;
        ...
        */
        ...
        for (var i = 0; i < ring.Count; i++)
        {
            ...
            if (!(Math.Max(stLong, endLong) >= longitude)) continue;
            ...
            if (longit >= longitude && longit <= Math.Max(stLong, endLong)) result = !result;
        }
    ...
    /// </summary>
    internal static void BoundingBox(this Polygon polygon, out float north, out float east, out float south,
        out float west)
    {
    ...
    /// </summary>
    internal static void BoundingBox(this List<Coordinate> exteriorRing, out float north, out float east,
