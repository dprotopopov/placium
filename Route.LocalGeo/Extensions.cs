...
public static class Extensions
{
        ...
        int last)
    {
        ...
        if (first + 1 != last)
        {
            ...
            for (var idx = first + 1; idx < last; idx++)
            {
            ...
            if (foundIndex > 0 && maxDistance > epsilonInMeter)
            {
                ...
                for (var idx = 0; idx < before.Count - 1; idx++) result.Add(before[idx]);
                for (var idx = 0; idx < after.Count; idx++) result.Add(after[idx]);
                return result;
            }
        }

        return new List<Coordinate>(new[] { points[first], points[last] });
    }
    ...
    public static Coordinate[] SimplifyBetween(this Coordinate[] points, float epsilonInMeter, int first, int last)
    {
        ...
        if (first + 1 != last)
        {
            ...
            if (foundIndex > 0 && maxDistance > epsilonInMeter)
            {
                ...
                for (var idx = 0; idx < before.Length - 1; idx++) result[idx] = before[idx];
                for (var idx = 0; idx < after.Length; idx++) result[idx + before.Length - 1] = after[idx];
                return result;
            }
        }

        return new[] { points[first], points[last] };
    }
    ...
    public static Polygon ToPolygon(this Box box)
    {
        return new Polygon
        {
            ExteriorRing = new List<Coordinate>(new[]
            {
                new(box.MinLat, box.MinLon),
    ...
    /// </summary>
    public static void BoundingBox(this List<Coordinate> exteriorRing, out float north, out float east, out float south,
        out float west)
    {
    ...
    {
        if (forward) return LocationAfterDistance(line.Coordinate1, line.Coordinate2, offset);
    ...
    ///     Note that polygons spanning a pole, without a point at the pole itself, will fail to detect points within the
    ///     polygon;
    ...
    {
        var internalArea = 0f;
        foreach (var ring in poly.InteriorRings) internalArea += ring.SurfaceArea();
