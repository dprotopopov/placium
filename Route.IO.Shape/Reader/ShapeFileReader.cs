...
namespace Route.IO.Shape.Reader
{
    ...
    public class ShapefileReader : AlgorithmBase
    {
        ...
        protected override void DoRun(CancellationToken cancellationToken)
        {
            ...
            for (int readerIdx = 0; readerIdx < _shapefileReaders.Count; readerIdx++)
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (progress != latestProgress)
                    {
                        ...
                        Logger.Log("ShapeFileReader", TraceEventType.Information,
            ...
            for (int readerIdx = 0; readerIdx < _shapefileReaders.Count; readerIdx++)
            {
                ...
                while (reader.Read())
                {
                    ...
                    { // the node has not been processed yet.
                        ...
                        if (profileId >= EdgeDataSerializer.MAX_PROFILE_COUNT)
                        {
                        ...
                        if (vertex1 != vertex2)
                        {
                            ...
                            { // edge is too long to fit into the network, adding an itermediate vertex.
                                ...
                                while (i < shape.Count)
                                {
                                    ...
                                    { // ok, previous shapepoint was the maximum one.
                                        ...
                                        // add edge.
                                        _routerDb.Network.AddEdge(vertex1, shortVertex, new EdgeData()
                                        {
                                ...
                                // add edge.
                                _routerDb.Network.AddEdge(vertex1, vertex2, new EdgeData()
                                {
                                    Distance = (float)shortDistance,
                                    MetaId = metaId,
                                    Profile = (ushort)profileId
                            ...
                            {
                                this.AddEdge(vertex1, vertex2, new EdgeData()
                                {
                                    Distance = distance,
                                    MetaId = metaId,
                                    Profile = (ushort)profileId
            ...
            // sort the network.
            Logger.Log("ShapeFileReader", TraceEventType.Information, "Sorting vertices...");
            _routerDb.Sort();
        ...
        private void AddEdge(uint vertex1, uint vertex2, EdgeData edgeData, List<Coordinate> intermediates)
        {
            ...
            { // everything is fine, no edge exists yet.
                _routerDb.Network.AddEdge(vertex1, vertex2, edgeData, new ShapeEnumerable(intermediates));
            }
            else
                ...
                { // try and use intermediate points if any.
                  // try and use intermediate points.
                    ...
                    { // intermediates found, use the first intermediate as the core-node.
                        ...
                        // add first part.
                        _routerDb.Network.AddEdge(vertex1, newCoreVertex, new EdgeData()
                        {
                            MetaId = splitMeta,
                        ...
                        _routerDb.Network.AddEdge(newCoreVertex, vertex2, new EdgeData()
                        {
                            MetaId = splitMeta,
                    ...
                    { // no intermediate or shapepoint found in either one. two identical edge overlayed with different profiles.
                      ...
                      // connect them with an edge of length '0'.
                        ...
                        _routerDb.Network.AddEdge(vertex1, newFromVertex, new EdgeData()
                        {
                            Distance = 0,
                            MetaId = splitMeta,
                            Profile = (ushort)splitProfile
                        ...
                        _routerDb.Network.AddEdge(newToVertex, vertex1, new EdgeData()
                        {
                            Distance = 0,
                            MetaId = splitMeta,
                            Profile = (ushort)splitProfile
                        ...
                        _routerDb.Network.AddEdge(newFromVertex, newToVertex, new EdgeData()
                        {
