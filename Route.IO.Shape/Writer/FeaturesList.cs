...
namespace Route.IO.Shape.Writer
{
    public class FeaturesList : IList<IFeature>
    {
        ...
        private IFeature BuildFeature(int index)
        {
            if (((index - 1) % 10000) == 0)
            {
                Logger.Log("FeatureList", TraceEventType.Information,
            ...
            {
                length += global::Route.LocalGeo.Coordinate.DistanceEstimateInMeter((float)coordinates[i + 0].Y, (float)coordinates[i + 0].X,
                    (float)coordinates[i + 1].Y, (float)coordinates[i + 1].X);
            }

            var tags = new AttributeCollection(_routerDb.EdgeProfiles.Get(edge.Data.Profile));
